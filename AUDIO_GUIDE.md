...
- **Cartesia ink-whisper STT** (Speech-to-Text) for transcribing user audio
- **AWS Polly TTS** (Text-to-Speech) for generating AI responses

## User Speech → Text (Cartesia ink-whisper STT)
...
4. **Backend automatically transcribes** using Cartesia ink-whisper
...
- Check network connectivity to Cartesia API

### "Cartesia API error"
- Verify CARTESIA_API_KEY is valid (get from https://play.cartesia.ai/keys)
...
processor = AudioProcessor(
    ...
    aws_access_key_id="your_aws_key",  # Required for TTS
    aws_secret_access_key="your_aws_secret"
)
